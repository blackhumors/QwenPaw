...
class BuiltInConfig(BaseModel):
    ...
    api_key: str = Field(
        default="",
        description="内置的 API Key",
    )
    simplified_mode: bool = Field(
        default=True,
        description="简化模式 - 隐藏非必要菜单",
    )
    developer_mode: bool = Field(
        default=False,
        description="开发者模式 - 显示所有菜单",
    )
...
class Config(BaseModel):
    ...
    # 简化模式配置
    builtin: Optional[BuiltInConfig] = Field(
        default=None,
        description="内置配置 - 用于开箱即用版本",
    )
