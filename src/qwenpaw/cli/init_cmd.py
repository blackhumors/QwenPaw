...
@click.option(
    ...
    "use_builtin",
    is_flag=True,
    help="Use built-in config (for packaged distribution).",
)
    ...
    if use_builtin:
        ...
        if builtin_api_key:
            ...
            try:
                ...
                provider = manager.get_provider(builtin_provider)
                if provider:
                    provider.api_key = builtin_api_key
                    # 保存到加密存储
                    ...
                    # 激活默认模型
                    asyncio.run(manager.activate_model(builtin_provider, builtin_model))
                    click.echo(f"✓ 内置 API Key 已配置: {builtin_provider} / {builtin_model}")
                else:
                    click.echo(f"⚠ 未找到 provider: {builtin_provider}")
            except Exception as e:
                click.echo(f"⚠ 配置内置 API Key 失败: {e}")
        else:
            click.echo("⚠ 未设置 QWENPAW_BUILTIN_API_KEY 环境变量")
