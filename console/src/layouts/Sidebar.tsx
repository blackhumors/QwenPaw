...
export default function Sidebar({ selectedKey }: SidebarProps) {
  ...
  const simplifiedMode = !developerMode;
  ...
  // 加载简化模式状态
  // (managed by DeveloperModeContext)
  ...
  // ── 简化模式过滤 ─────────────────────────────────────────────────────────
  // 非开发者模式下只显示 chat
  const simplifiedMenuKeys = simplifiedMode ? ["chat"] : [];

  const filterMenuItems = (items: MenuProps["items"]) => {
    if (!simplifiedMode || developerMode) return items;
    return items?.map((item) => {
      ...
      if ("children" in item && item.children) {
        const filteredChildren = item.children.filter((child) => {
          ...
          const key = child.key;
          return simplifiedMenuKeys.includes(String(key));
        });
      ...
      // 叶子节点
      const key = item.key;
      return simplifiedMenuKeys.includes(String(key)) ? item : null;
    }).filter(Boolean);
  };

  const filteredAgentMenuItems = filterMenuItems(agentMenuItems);
  const filteredSettingsMenuItems = filterMenuItems(settingsMenuItems);

  const filteredCollapsedNavItems = simplifiedMode && !developerMode
    ? collapsedNavItems.filter((item) =>
        simplifiedMenuKeys.includes(item.key)
      )
    : collapsedNavItems;
    ...
    >
      {collapsed ? (
        <nav className={styles.collapsedNav}>
          {filteredCollapsedNavItems.map((item) => {
        ...
        <>
          ...
          <div className={styles.agentScopedSection}>
            <div className={styles.agentSelectorContainer}>
              {!simplifiedMode && <AgentSelector collapsed={collapsed} />}
